...
# install spectre-metdown-checker for verification
cd /usr/local/src
...
# to fix spectre v2 in the linux kernel we need to recompile the kernel with gcc 7.3.1+
...
# FIXME most likely needs a reboot to load the new kernel and to show up-to-date info
sudo /usr/local/src/spectre-meltdown-checker/spectre-meltdown-checker.sh
